...
| [`0009-journal-store-and-sinks.md`](0009-journal-store-and-sinks.md) | Provisional, amended by 0014 | One JSON Lines file per run under a state directory, hash-chained; projections computed on read; a local sink first, the orphan branch as the default off-machine sink and an object store as an optional one. |
...
| [`0011-the-check-is-the-unit.md`](0011-the-check-is-the-unit.md) | Accepted | A check is a moon task with tags; five checkpoints from the harness hook to the schedule; moon under every factory check; no fixed time budget; scheduled checks are checks and model-driven upkeep becomes issues. |
| [`0012-slots-check-recognisers-and-slot-attestations.md`](0012-slots-check-recognisers-and-slot-attestations.md) | Accepted | A slot counts as filled when a check recogniser confirms the adopter's check or a slot attestation covers it; empty adopter-only slots warn; one table per slot in `osf.toml`; one TOML file per ecosystem; generated factory-owned files with a drift gate. |
| [`0013-the-aggregation-check.md`](0013-the-aggregation-check.md) | Accepted | One required check that reads every other check on the commit, finds result files by content, and posts a slot table rendered from one journal event; runs as a workflow in the adopter's repository; reviews are checks with reported evidence. |
| [`0014-the-journal-at-every-checkpoint.md`](0014-the-journal-at-every-checkpoint.md) | Accepted | Every checkpoint writes events; a local buffer flushes on push and on a timer; the orphan branch is the default sink and an object store the optional one; raw transcripts share the path. |
| [`0015-suppressions.md`](0015-suppressions.md) | Accepted | A factory marker and the ecosystem's native markers both count; the factory marker carries a reason and an expiry; a native suppression a change adds is a finding. |
