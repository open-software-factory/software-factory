...
Date: 2026-08-16, revised 2026-09-15 before any engine code existed. The original record chose Go; the revision replaces the decision in place rather than superseding it, because nothing had been built against it.
...
Build the Factory Engine, its command-line interface and the verifier runner in Rust, as one static binary per platform. Prefer explicit types, ordinary crates and standard process or protocol boundaries. Do not use a Rust-specific plugin mechanism as the primary integration model; integrations stay out of process.

## Consequences

- The engine and CLI ship as native, cross-compiled binaries with no runtime dependency.
- Integrations may use any language and normally communicate out of process, per decision 0002.
...
- Fewer coding agents and contributors write idiomatic Rust than Go, and the cheaper models the factory routes routine work to are weakest there. Keep the engine small and every adapter out of process so that most contributed code is not Rust.
- Compile times are longer than Go's. The first slice is kept deliberately small so it stays cheap to delete.
